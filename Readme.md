...
This is a fork from Andrej Karpathy's [char-rnn](https://github.com/karpathy/char-rnn), modified for truecasing.

sample.py performs beam search for possible upcasing of input (instead of unconstrained generation), unless you set '-stdin 0', in which case it behaves like

The beam search implementation is taken from raymondhs' code for the EMNLP'16 paper: Learning to Capitalize with Character-Level Recurrent Neural Networks: An Empirical Study.



## opt.min_freq
[From zhangzbin] also add an option called 'min_freq' to exclude characters with low count (treating them as <unk>) because the vocab size in Chinese is very big

## Scheduled Sampling
[From zhangzbin]
Samy+Bengio's paper [Scheduled Sampling for Sequence Prediction with Recurrent Neural Networks](http://arxiv.org/abs/1506.03099) in NIPS15 proposes gradually using 1best model context instead of gold context. This helps reduce overfitting. The amount of real data linearly decays.

(on by default) `-use_ss 1` enables scheduled sampling. `-start_ss` is the beginning probability of using real data (default 1, recommended). `-min_ss` (should be >0) is the ending probability of using real data - don't set it too low or the noise will hurt performance. Finally, `-decay_ss` is the linear decay rate.
